...
import { getMaterials, uploadMaterial, deleteMaterial } from "../../services/material.service";
...
const Materials = ({ role }) => {
  ...
  const fetchSubjects = async () => {
    try {
      const res = role === "ADMIN" ? await getSubjects() : await getFacultyAssignments();
  ...
  return (
    <div className="p-8 pb-24">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1 font-medium font-sans">
            Access and manage academic resources
          </p>
        </div>
        {role === "FACULTY" && (
          <button
      ...
      <div className="flex items-center gap-4 mb-8 bg-blue-50/50 p-4 rounded-2xl border border-blue-100">
        ...
        <span className="text-sm font-bold text-[#003B73] uppercase tracking-wider">
          Filter by Subject:
        </span>
        <CustomSelect
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {materials.length > 0 ? (
            materials.map((mat, idx) => (
              <div
                key={idx}
                className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-xl transition-all duration-300 animate-fadeIn"
              ...
              >
                <div className="p-6">
                  ...
                  <h3 className="font-black text-[#003B73] text-lg mb-1 group-hover:translate-x-1 transition-transform">
                    {mat.title}
                  </h3>
                  <p className="text-xs font-bold text-gray-500 mb-3 flex items-center gap-1">
                    <BookOpen size={12} /> {mat.subject?.code} •{" "}
                    {mat.subject?.name}
                  </p>
                  <p className="text-sm text-gray-600 line-clamp-2 mb-4 font-sans">
                    {mat.description || "No description provided."}
                  </p>

                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-50">
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                      Added by {mat.instructor?.fullName}
                    </span>
                    {mat.fileUrl && (
                      <a
                        href={mat.fileUrl}
