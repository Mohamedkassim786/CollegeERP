...
import { Bell, Plus, Trash2, Filter, Megaphone, Calendar, User } from 'lucide-react';
        ...
        <div className="p-8 pb-24">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 font-medium italic">Stay updated with latest college notifications</p>
                </div>
                ...
                <div className="grid gap-6">
                    {announcements.length > 0 ? (
                        announcements.map((ann, idx) => (
                            <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all group animate-fadeIn" style={{ animationDelay: `${idx * 100}ms` }}>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#003B73] transition-colors">{ann.title}</h3>
                                            ...
                                            <div className="flex items-center gap-4 text-xs text-gray-500 mt-1 font-medium">
                                                ...
                                                <span className="flex items-center gap-1"><User size={12} /> {ann.author?.fullName} ({ann.author?.role})</span>
                                            </div>
                                        </div>
                                    </div>
                                    {(role === 'ADMIN' || ann.postedBy === ann.postedBy) && ( // Author check logic to be refined if needed
                                        <button onClick={() => handleDelete(ann.id)} className="text-gray-300 hover:text-red-500 transition-colors p-2 rounded-lg hover:bg-red-50">
                                            <Trash2 size={18} />
                                        </button>
                                    )}
                                </div>
                                <p className="text-gray-600 leading-relaxed font-sans">{ann.content}</p>
                                {ann.department && (
                                    <div className="mt-4 flex items-center gap-2">
                                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Target:</span>
                                        <span className="bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase">{ann.department}</span>
                                    </div>
                                )}
                            </div>
                        ))
                        ...
                        <div className="bg-white rounded-3xl p-16 text-center shadow-inner border border-dashed border-gray-200">
                            ...
                            <p className="text-xl font-bold text-gray-400">No announcements yet</p>
                        </div>
                    )
                    }
                </div>
