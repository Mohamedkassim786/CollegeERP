...
const HallAllocation = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const allocRes = await api.get(
        `admin/hall-allocation/sessions/${selectedSession.id}/allocations`,
      );
  ...
  const toggleLock = async (session) => {
    try {
      await api.patch(`admin/hall-allocation/sessions/${session.id}/lock`, {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const response = await api.get(
        `admin/hall-allocation/sessions/${selectedSession.id}/export`,
        {
          responseType: "blob",
        },
      );
  ...
  const handleExportGridPDF = async () => {
    ...
    try {
      const response = await api.get(
        `admin/hall-allocation/sessions/${selectedSession.id}/export-grid`,
        {
          responseType: "blob",
        },
      );
  ...
  const previewAllocations = async (session) => {
    try {
      const res = await api.get(
        `admin/hall-allocation/sessions/${session.id}/allocations`,
      );
  ...
  const handleSaveSubjects = async () => {
    ...
    try {
      const res = await api.put(
        `admin/hall-allocation/sessions/${selectedSession.id}/subjects`,
        {
