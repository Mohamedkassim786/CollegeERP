...
import { Upload, FileSpreadsheet, Download, AlertCircle, CheckCircle2, X, Trash2 } from 'lucide-react';
...
const ArrearManagement = () => {
    ...
    const handleDeleteArrear = async (id) => {
        if (!window.confirm('Are you sure you want to remove this arrear record? This will also delete all associated attempts.')) {
            return;
        }
        ...
        try {
            ...
            toast.error(err.response?.data?.message || 'Failed to delete record', { id: 'delete-arrear' });
        }
    };
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            toast.success(response.data.message || 'Upload successful', { id: 'upload-arrear' });
            setUploadResult({
                success: true,
            ...
            const errorMessage = error.response?.data?.message || 'Failed to upload arrear records';
    ...
    return (
        <div className="p-8 pb-24">
            <div className="mb-8 flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-black text-[#003B73]">Arrear Management</h1>
                    <p className="text-gray-500 font-medium tracking-wide">Manage historical arrear records and import them into the current exam session.</p>
                </div>
                <div>
                    <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Upload Section */}
                <div className="lg:col-span-1 border-2 border-dashed border-gray-300 rounded-3xl p-8 bg-gray-50 flex flex-col items-center justify-center text-center relative"
                ...
                >
                    {!file ? (
                        <>
                            ...
                            <h3 className="text-xl font-bold text-[#003B73] mb-2">Drag & Drop Excel</h3>
                            <p className="text-gray-500 font-medium mb-6">or click below to browse files</p>
                            ...
                            <div className="mt-8 text-sm text-gray-400 font-medium">
                                <p>Format: .xlsx, .xls</p>
                                <p>Fields: RegisterNumber, SubjectCode, Semester, InternalMarks, CurrentAttemptSemester</p>
                            </div>
                        </>
                    ) : (
                ...
                <div className="lg:col-span-2">
                    {uploadResult ? (
                                    ...
                                    <div className="bg-red-50 rounded-xl p-4 max-h-[400px] overflow-y-auto">
                                        <ul className="space-y-2">
                                            {uploadResult.errors.map((err, i) => (
                                                <li key={i} className="text-red-700 font-medium text-sm flex items-start gap-2">
                        ...
                        <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 h-full flex flex-col justify-center">
                            <h2 className="text-2xl font-black text-[#003B73] mb-6">Instructions</h2>
                            <ul className="space-y-4 font-medium text-gray-600">
                                <li className="flex gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
                                    <div className="w-8 h-8 bg-blue-100 text-[#003B73] rounded-lg flex items-center justify-center font-black shrink-0">1</div>
                                    <p>Download the standard template by clicking the <strong className="text-gray-800">TEMPLATE</strong> button.</p>
                                </li>
                                <li className="flex gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
                                    <div className="w-8 h-8 bg-blue-100 text-[#003B73] rounded-lg flex items-center justify-center font-black shrink-0">2</div>
                                    <p>Fill in the Register Numbers and the corresponding Subject Codes of the failed subjects.</p>
                                </li>
                                <li className="flex gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
                                    <div className="w-8 h-8 bg-blue-100 text-[#003B73] rounded-lg flex items-center justify-center font-black shrink-0">3</div>
                                    <div>
                                        <p>Important: <strong className="text-gray-800">Semester</strong> is the original semester of the subject (e.g., 4). <strong className="text-gray-800">CurrentAttemptSemester</strong> is the semester the student is currently writing it in (e.g., 6).</p>
                                    </div>
                                </li>
                                <li className="flex gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
                                    <div className="w-8 h-8 bg-blue-100 text-[#003B73] rounded-lg flex items-center justify-center font-black shrink-0">4</div>
                                    <p>Internal marks are optional, but recommended to ensure automatic carry-over evaluation.</p>
                                </li>
                            </ul>
                        </div>
                    )}
            ...
            <div className="mt-12 bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                <h2 className="text-xl font-bold text-[#003B73] mb-6 border-b pb-4">Currently Registered Arrears</h2>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-[#003B73] text-white">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-bold tracking-wider">Current Attempt</th>
                                    <th className="px-6 py-4 font-bold tracking-wider text-center">Attempts</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {arrears.map((a, i) => (
                                    <tr key={i} className="hover:bg-blue-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 font-bold text-gray-600 truncate max-w-[200px]">{a.student?.name || 'Unknown'}</td>
                                        <td className="px-6 py-4 font-bold text-[#003B73]">{a.subject?.code || 'N/A'}</td>
                                        <td className="px-6 py-4 font-semibold text-gray-600">{a.semester}</td>
                                        <td className="px-6 py-4 font-bold text-indigo-600">
                                            {a.attempts?.filter(att => !att.resultStatus).map(att => att.semester).join(', ') || '-'}
                                        </td>
                                        <td className="px-6 py-4 text-center font-bold text-gray-700">{a.attemptCount}</td>
                                        <td className="px-6 py-4">
                                            {a.isCleared ? (
                                                <span className="px-3 py-1 bg-green-100 text-green-700 rounded-lg font-bold text-xs">Cleared</span>
                                            ) : (
