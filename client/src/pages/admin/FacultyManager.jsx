...
const FacultyManager = () => {
  ...
  const [selectedFacultyForAbsence, setSelectedFacultyForAbsence] =
    useState(null);
  ...
  const fetchAbsences = async () => {
    try {
      const res = await api.get("/admin/faculty-absences", {
  ...
  const fetchDailySchedule = async (facultyId, date) => {
    try {
      ...
      const [timetableRes, subsRes] = await Promise.all([
        ...
        api.get("/admin/substitutions", {
          params: { date, originalFacultyId: facultyId },
        }),
      ]);
  ...
  const handleMarkAbsent = async () => {
    ...
    try {
      ...
      alert(
        `Marked ${selectedFacultyForAbsence.fullName} as absent on ${absenceDate}`,
      );
  ...
  const handleRevertSelected = async () => {
    ...
    if (
      !confirm(
        `Revert ${selectedSubsToRemove.length} substituted periods to normal?`,
      )
    )
      return;
    ...
    try {
      ...
      await Promise.all(
        selectedSubsToRemove.map((id) =>
          api.delete(`/admin/substitutions/${id}`),
        ),
      );
  ...
  const handleRemoveFullAbsence = async () => {
    ...
    try {
      ...
      alert(
        `Absence removed for ${selectedFacultyForAbsence.fullName}. Schedule reverted.`,
      );
      setAbsenceModalOpen(false);
      if (absenceDate === viewDate) fetchAbsences();
    } catch (err) {
    ...
    <div className="flex flex-col animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">
            Manage academic staff and daily attendance status.
          </p>
        </div>
                      ...
                      >
                        <td className="px-8 py-6 text-left">
                          <div className="flex items-center gap-4">
                            <div
                              className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg shadow-sm transition-transform group-hover:scale-110 ${isAbsent
                        ...
                        <td className="px-8 py-6 text-right">
                          <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              title={
                                isAbsent ? "Manage Absence" : "Mark Absent"
                              }
                            >
                              <CalendarX size={20} />
                            </button>
      ...
      {absenceModalOpen && selectedFacultyForAbsence && (
        <div className="fixed inset-0 bg-[#003B73]/20 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-fadeIn">
          <div className="bg-white rounded-[48px] shadow-2xl w-full max-w-xl border border-gray-100">
            <div className="p-10 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-gray-500 font-bold text-sm mt-1">
                  {selectedFacultyForAbsence.fullName} • {absenceDate}
                </p>
              </div>
              <button
              ...
              {!absentFacultyIds.includes(selectedFacultyForAbsence.id) ? (
                <div className="space-y-8">
                  <div className="bg-orange-50 p-6 rounded-[32px] border border-orange-100 flex items-start gap-4">
                    ...
                    <div>
                      <p className="font-black text-orange-900 leading-tight">
                        Proceed with Caution
                      </p>
                      <p className="text-sm text-orange-700 font-bold mt-1 leading-relaxed">
                        Marking a faculty as absent will flag all their
                        scheduled classes for this date as cancelled or
                  ...
                  <div className="flex gap-4">
                    <button
                      onClick={() => setAbsenceModalOpen(false)}
                      className="flex-1 py-5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-[24px] font-black transition-all"
                    >
                      Go Back
                    </button>
                    <button
                      onClick={handleMarkAbsent}
                      className="flex-1 py-5 bg-orange-600 text-white rounded-[24px] font-black hover:bg-orange-700 shadow-xl shadow-orange-900/20 transform active:scale-95 transition-all"
                    >
                      Confirm Absence
                    </button>
                  </div>
                ...
                <div className="space-y-8">
                      ...
                      {facultySchedule.length === 0 ? (
                        <div className="text-center py-10">
                          ...
                          <p className="text-xs font-bold text-gray-300">
                            Schedule is empty for this date.
                          </p>
                        </div>
                      ) : (
                        facultySchedule.map((slot) => (
                          <div
                            key={slot.id}
                            className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm group hover:shadow-md transition-all"
                          >
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <div
                                className={`text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest ring-1 ${slot.substitution
                              ...
                              <div className="flex items-center justify-between p-3 bg-emerald-50 rounded-2xl">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs font-black text-emerald-800">
                                    {
                                      slot.substitution.substituteFaculty
                                        .fullName
                                    }
                                  </span>
                                </div>
                              ...
                              <div className="flex gap-3">
                                ...
                                >
                                  ...
                                  {facultyList
                                    .filter(
                                      (f) =>
                                        f.id !== selectedFacultyForAbsence.id &&
                                        !absentFacultyIds.includes(f.id),
                                    )
                                    .map((f) => (
                                ...
                                <button
                                  onClick={() =>
                                    handleAssignSubstitute(slot.id)
                                  }
                  ...
                  <div className="pt-6 border-t border-gray-100">
                    <button
                      onClick={handleRemoveFullAbsence}
                      className="w-full group py-6 bg-red-50 text-red-600 rounded-[32px] font-black hover:bg-red-600 hover:text-white transition-all shadow-sm hover:shadow-red-200 border border-red-100 flex flex-col items-center justify-center gap-1 group"
                    >
                      <div className="flex items-center gap-2">
                        <Trash2 size={20} />
                      ...
                      <span className="text-[10px] opacity-80 uppercase tracking-[0.2em]">
                        {activeSubstitutions.length > 0
                          ? `Revoke ${activeSubstitutions.length} substitutions & Presence Reset`
                          : "Mark as Present"}
                      </span>
                    </button>
                  </div>
