...
import { getMarksForAdmin, updateMarksForAdmin } from "../../../services/marks.service";
import {
  ChevronLeft,
  Save,
  Edit3,
...
const AdminMarksManager = () => {
  ...
  const fetchMarks = async () => {
    setLoading(true);
    try {
      const res = await getMarksForAdmin(subjectId);
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-gray-500 font-medium mt-1">
              Direct marks manipulation for institutional compliance and
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-black text-[#003B73] tracking-tight uppercase">
                Marks Distribution Channel
              </h2>
              <p className="text-gray-400 font-bold text-xs uppercase tracking-widest mt-1">
                Cross-Semester Performance Ledger
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="overflow-hidden bg-gray-50/30 rounded-3xl border border-gray-100 relative z-10">
            <table className="w-full text-center border-collapse">
              <thead className="bg-gray-100/50 text-[#003B73] text-[10px] font-black uppercase tracking-[0.2em]">
                <tr>
                  <th className="px-8 py-8 text-left border-r border-gray-200/50 bg-gray-100/30 w-48">
                    Roll No
                  </th>
              ...
              <tbody className="divide-y divide-gray-100">
                {students.map((student) => {
                  const c1 = sum(
                    student.marks.cia1_test,
                    student.marks.cia1_assignment,
                    student.marks.cia1_attendance,
                  );
                  const c2 = sum(
                    student.marks.cia2_test,
                    student.marks.cia2_assignment,
                    student.marks.cia2_attendance,
                  );
                  const c3 = sum(
                    student.marks.cia3_test,
                    student.marks.cia3_assignment,
                    student.marks.cia3_attendance,
                  );
            ...
            <div className="bg-white p-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                {[1, 2, 3].map((phase) => (
                  <div
                    key={phase}
                    className="p-8 bg-gray-50 rounded-[40px] border border-gray-100 space-y-8"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-[#003B73] text-white rounded-xl flex items-center justify-center font-black text-sm">
                        0{phase}
                      </div>
                      <h4 className="font-black text-[#003B73] uppercase tracking-widest text-sm">
                        CIA Phase
                      </h4>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleFormChange(`cia${phase}_test`, e.target.value)
                          }
                        />
                      </div>
                      <div className="space-y-2">
                    ...
                    <div className="pt-4 border-t border-gray-200/50">
                      <div className="flex justify-between items-center px-1">
                        ...
                        <span className="text-xl font-black text-[#003B73]">
                          {sum(
                            editForm[`cia${phase}_test`],
                            editForm[`cia${phase}_assignment`],
                            editForm[`cia${phase}_attendance`],
                          )}
                        </span>
                      </div>
                    </div>
