        ...
        <div className="space-y-8 animate-fadeIn">
            {view === 'depts' ? (
                ...
                <StaffListView 
                    ...
                    onAddFaculty={() => { setModalRole('FACULTY'); setEditingStaff(null); setShowStaffModal(true); }}
                    onEdit={(staff) => { setEditingStaff(staff); setShowStaffModal(true); }}
                    onDelete={async (id) => {
...
const StaffModal = ({ departments, facultyList, onClose, onSave, editingStaff }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        computedRoles: editingStaff?.computedRoles || ['FACULTY'],
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Object.keys(formData).forEach(key => {
                if (key === 'computedRoles') {
                    data.append('computedRoles', JSON.stringify(formData.computedRoles));
                } else {
    ...
    return (
        <div className="fixed inset-0 bg-[#003B73]/20 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-fadeIn">
            <div className="bg-white rounded-[48px] w-full max-w-4xl shadow-2xl border border-gray-100 max-h-[90vh] flex flex-col overflow-hidden animate-scaleIn">
                <div className="flex justify-between items-center p-10 pb-4 shrink-0">
                    <div>
                        <h3 className="text-3xl font-black text-[#003B73] tracking-tight">
                            {editingStaff ? 'Edit Staff Profile' : 'Register New Faculty'}
                        </h3>
                ...
                <div className="flex-1 overflow-y-auto px-10 pb-10 custom-scrollbar scrollbar-gutter-stable my-2">
                    <form onSubmit={handleSubmit} className="space-y-8 pt-4">
                        ...
                        <div className="flex flex-col items-center justify-center p-8 bg-gray-50 rounded-[32px] border-2 border-dashed border-gray-200 hover:border-[#003B73] transition-all group relative overflow-hidden">
                            {photoPreview ? (
                                <div className="relative w-32 h-32">
                                    <img src={photoPreview} alt="Preview" className="w-full h-full object-cover rounded-[24px] shadow-lg" />
                                    <button 
                                        type="button"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="flex flex-col gap-2 bg-gray-50 p-3 rounded-[22px] border border-gray-100">
                                    <label className="flex items-center gap-3 p-2 cursor-pointer group/role">
                                        <input type="checkbox" className="w-5 h-5 accent-[#003B73]" 
                                            ...
                                            onChange={(e) => {
                                                const roles = new Set(formData.computedRoles);
                                                e.target.checked ? roles.add('HOD') : roles.delete('HOD');
                                    ...
                                    <label className="flex items-center gap-3 p-2 cursor-pointer group/role">
                                        <input type="checkbox" className="w-5 h-5 accent-[#003B73]" 
                                            ...
                                            onChange={(e) => {
                                                const roles = new Set(formData.computedRoles);
                                                e.target.checked ? roles.add('FIRST_YEAR_COORDINATOR') : roles.delete('FIRST_YEAR_COORDINATOR');
                                                setFormData({...formData, computedRoles: Array.from(roles)});
                                            }}
                                        />
                                        <div className="flex flex-col flex-1">
                            ...
                            <div>
                                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3 px-1">Blood Group</label>
                                <CustomSelect 
                                    className="w-full"
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3 px-1">Full Address</label>
                                <textarea rows="3" className="input-field min-h-[120px] py-5 resize-none" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-amber-50 p-6 rounded-[24px] border border-amber-100 flex gap-4">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-amber-900 uppercase tracking-widest leading-none mb-2">Default Password</p>
                                    <p className="text-sm text-amber-700 font-bold">All staff accounts use: <span className="font-mono text-amber-900 bg-white px-2 py-0.5 rounded shadow-sm">password123</span></p>
                                </div>
                            </div>

                            {formData.computedRoles?.includes('HOD') && (
                                <div className="bg-purple-50 p-6 rounded-[24px] border border-purple-100 flex gap-4 animate-fadeIn">
                                    <div className="p-3 bg-white rounded-2xl shadow-sm text-purple-600">
                                        <Shield size={24} />
                                    </div>
                                    <p className="text-sm text-purple-700 font-bold self-center">Assigned HOD will safely replace the existing HOD of the department.</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex gap-4 pt-6">
                            <button 
                                type="button" 
                                onClick={onClose} 
                                className="flex-1 py-5 bg-gray-100 text-gray-500 rounded-3xl font-black text-xs uppercase tracking-[0.2em] hover:bg-gray-200 transition-all hover:scale-[1.02]"
                            >
                                Cancel
                            </button>
                            <button 
                                disabled={loading} 
                                type="submit" 
                                className="flex-[2] py-5 bg-[#003B73] text-white rounded-3xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:scale-[1.02] transition-all shadow-xl shadow-blue-900/20 active:scale-95 disabled:opacity-50"
                            >
                                {loading ? <RefreshCw size={20} className="animate-spin" /> : (editingStaff ? 'Save Changes' : `Register Faculty`)}
                            </button>
                        </div>
                    </form>
                </div>
...
const InputField = ({ label, value, onChange, placeholder, required = false, type = "text" }) => (
    <div>
        <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3 px-1">{label}</label>
        <input 
            type={type}
            required={required}
