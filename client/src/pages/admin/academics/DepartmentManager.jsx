...
const DepartmentManager = () => {
  ...
  const [showBanner, setShowBanner] = useState(() => {
    return !localStorage.getItem("dept_degree_banner_dismissed");
  });
    ...
    <div className="flex flex-col animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6 px-2">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">
            Configure institutional structure and assign leadership roles.
          </p>
        </div>

        {showBanner && (
          <div className="flex-1 max-w-lg bg-emerald-50 border border-emerald-100 p-4 rounded-3xl flex items-center justify-between gap-4 animate-fadeIn">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-bold text-emerald-800">
                Departments now have a Degree Type field. Please review and update if needed.
              </p>
            </div>
            <button 
              onClick={() => {
                setShowBanner(false);
                localStorage.setItem("dept_degree_banner_dismissed", "true");
              }}
      ...
      {confirmState && (
        <div className="fixed inset-0 bg-[#003B73]/20 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-fadeIn">
          <div className="bg-white rounded-[48px] w-full max-w-md p-10 relative shadow-2xl border border-red-50 flex flex-col items-center text-center">
            ...
            <h3 className="text-3xl font-black text-red-900 mb-3 tracking-tighter uppercase">Confirm Deletion</h3>
            <p className="text-red-700/80 font-bold text-base mb-10 max-w-sm">{confirmState.message}</p>
            
            <div className="flex gap-4 w-full">
              <button
                onClick={() => setConfirmState(null)}
                className="flex-1 py-5 bg-gray-100 text-gray-500 rounded-3xl font-black text-xs uppercase tracking-[0.2em] hover:bg-gray-200 transition-all font-black"
              >
                Cancel
              </button>
              <button
                onClick={() => { handleDelete(confirmState.id); setConfirmState(null); }}
                className="flex-1 py-5 bg-red-600 text-white rounded-3xl font-black text-xs uppercase tracking-[0.2em] hover:bg-red-700 shadow-2xl shadow-red-900/20 transition-all hover:-translate-y-1 font-black"
              >
                Confirm
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-10">
        {(Array.isArray(departments) ? departments : [])
          .filter(d => d.name?.toLowerCase() !== 'first year' && d.code !== 'GEN' && d.code !== 'GEN1')
          .map((dept, index) => (
            <div
              key={dept.id}
              ...
              <div className="p-8">
                <div className="flex justify-between items-start mb-6">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`px-3 py-1 ${dept.type === 'Support' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-purple-50 text-purple-600 border-purple-100'} text-[10px] font-black uppercase tracking-widest rounded-lg border`}>
                        {dept.type || "Academic"}
                      </span>
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group-hover:bg-white transition-colors duration-500">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p
                          className={`font-bold mt-1 ${dept.hodName === "Unassigned" || !dept.hodName ? "text-gray-400 italic" : "text-gray-800"}`}
                        >
                    ...
                    <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 group-hover:bg-white transition-colors duration-500 flex flex-col justify-center">
                      ...
                      <p className="font-extrabold text-lg text-gray-800 tracking-tight text-center">
                        {dept.name?.toLowerCase() === "first year" || dept.code === "GEN1" || dept.code === "GEN"
                          ? "1"
                          : dept.years || "2,3,4"}
                      </p>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-3 pt-6 border-t border-gray-100">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-xl font-black text-[#003B73]">
                      {dept.stats?.faculty || 0}
                    </p>
                  </div>
                  {dept.type !== 'Support' && (
        ...
        {departments.length === 0 && (
          <div className="col-span-full py-40 flex flex-col items-center text-center bg-white rounded-[40px] border-2 border-dashed border-gray-100 shadow-sm">
            ...
            <p className="text-gray-400 font-medium max-w-sm mx-auto mt-2 px-6">
              Register your institutional units to begin managing curriculum and
              faculty.
            </p>
          </div>
        )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-[#003B73]/20 backdrop-blur-md flex items-center justify-center p-6 z-[100] animate-fadeIn">
          <div className="bg-white rounded-[48px] w-full max-w-xl shadow-2xl border border-gray-100 transform animate-modalEnter">
            <div className="bg-white p-10 pb-4 rounded-[48px]">
              <div className="flex justify-between items-center mb-8">
                <div>
                  ...
                  <p className="text-gray-500 font-bold text-sm mt-1">
                    {editMode
                      ? "Update institutional unit parameters"
                      : "Register a new institutional unit"}
                  </p>
                </div>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      >
                        <option value="Academic">Academic (Has Students)</option>
                        <option value="Support">Support (Faculty Only)</option>
                      </CustomSelect>
                    </div>
                ...
                <div className="p-8 bg-gray-50 rounded-[32px] border border-gray-100">
                  <div className="flex flex-col justify-center items-center">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4 block px-1">
                      Academic Cycle (Years Covered)
                    </label>
                    <div className="flex gap-4">
                      {[1, 2, 3, 4].map((year) => (
                        <button
                          ...
                          className={`w-16 h-16 rounded-2xl font-black text-xl transition-all ${formData.years
                            .split(",")
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    >
                      ...
                      {(Array.isArray(facultyList) ? facultyList : []).map(
                        (f) => (
                          <option key={f.id} value={f.id}>
                            {f.fullName} ({f.department || "No Dept"})
                          </option>
                        ),
                      )}
                    </CustomSelect>
