...
const AdminHome = () => {
  ...
  const [firstYearStats, setFirstYearStats] = useState({ students: 0, sections: 0, coordinator: 'Not Assigned' });
    ...
    const fetchStats = async () => {
      try {
        ...
        if (res.data) {
          ...
          setFirstYearStats(res.data.firstYear || { students: 0, sections: 0, coordinator: 'Not Assigned' });
        }
      } catch (err) {
  ...
  const stats = [
    ...
    },
    {
      ...
      value:
        typeof statsData.avgAttendance === "number"
        ...
        {stats.map((stat, idx) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div
                className={`p-3 rounded-xl bg-gradient-to-br ${stat.gradient} shadow-lg text-white`}
              >
                <stat.icon size={24} />
              </div>
              <div
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Average marks over last 6 months
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 animate-fadeIn delay-300 lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="badge bg-blue-100 text-blue-700 border border-blue-200">
              Current Week
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 animate-fadeIn delay-100">
          ...
          <div className="space-y-3">
            <button
              onClick={() =>
                navigate("/admin/students", { state: { openAddModal: true } })
              }
        ...
        {/* First Year Overview Card */}
        <div className="bg-gradient-to-br from-[#003B73] to-blue-900 rounded-3xl shadow-xl shadow-blue-900/20 p-8 text-white animate-fadeIn delay-300 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16 transition-all group-hover:scale-150 duration-700"></div>
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="space-y-6 flex-1">
              <div className="flex items-center justify-between pb-4 border-b border-white/10">
                <span className="text-blue-100 font-bold opacity-80 uppercase text-[10px] tracking-widest">Total Students</span>
                <span className="text-3xl font-black">{firstYearStats.students}</span>
              </div>
              <div className="flex items-center justify-between pb-4 border-b border-white/10">
                <span className="text-blue-100 font-bold opacity-80 uppercase text-[10px] tracking-widest">Common Sections</span>
                <span className="text-3xl font-black">{firstYearStats.sections}</span>
              </div>
              <div>
                <span className="text-blue-100 font-bold opacity-80 uppercase text-[10px] tracking-widest block mb-2">Coordinator</span>
                <div className="flex items-center gap-3 bg-white/10 p-4 rounded-2xl border border-white/10">
                  <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center font-black">
                    {firstYearStats.coordinator.charAt(0)}
                  </div>
                  <div className="min-w-0">
                    <p className="font-black text-sm truncate">{firstYearStats.coordinator}</p>
                    <p className="text-[10px] font-bold text-blue-200 uppercase tracking-widest">First Year HOD</p>
                  </div>
                </div>
              </div>
            </div>
            <button
              onClick={() => navigate("/admin/students", { state: { category: 'FIRST_YEAR' } })}
              className="w-full mt-8 py-4 bg-white text-[#003B73] rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-50 transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2"
            >
              Manage First Year <ArrowUpRight size={16} />
            </button>
          </div>
        </div>
      </div>
