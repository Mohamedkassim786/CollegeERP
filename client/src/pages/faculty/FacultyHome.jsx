    ...
    <div className="min-h-screen bg-[#F5F7FA] p-6 custom-scrollbar">
              ...
              >
                {marksSubmission.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={
                      entry.name === "Submitted" || entry.name === "Completed"
                        ? "#003B73"
                        : "#93C5FD"
                    }
                  />
            ...
            {marksSubmission.map((item, idx) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div
                    className={`w-3 h-3 rounded-full ${item.name === "Submitted" || item.name === "Completed" ? "bg-[#003B73]" : "bg-blue-300"}`}
                  ></div>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 mb-6 animate-fadeIn delay-300">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Weekly attendance rate across all classes
          </p>
        </div>
          ...
          <div className="space-y-3">
            {pendingTasks.length > 0 ? (
              pendingTasks.map((item, idx) => (
                <div
                  key={idx}
                  className="p-3 bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow"
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span
                      className={`badge ml-2 ${item.priority === "high" ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"}`}
                    >
                      {item.priority}
                    </span>
                  </div>
