    ...
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="mb-6">
        <div className="flex justify-between text-sm mb-2">
          ...
          <span className="font-bold text-[#003B73]">
            {details.syllabusCompletion}%
          </span>
        </div>
...
const StudentsTab = ({ subjectId, details }) => {
          ...
          <tbody className="divide-y divide-gray-100">
            {filteredStudents.map((s) => (
              <tr key={s.id} className="hover:bg-gray-50">
                <td className="p-4 font-mono text-sm text-[#003B73] font-bold">
                  {s.rollNo}
                </td>
                <td className="p-4 font-medium text-gray-800">{s.name}</td>
                <td className="p-4 text-center">
                  <span
                    className={`font-bold ${s.attendancePercentage < 75 ? "text-red-600" : "text-emerald-600"}`}
                  >
...
const AttendanceTab = ({ subjectId, details }) => {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `Attendance_${subject.code}_${fromDate}_to_${toDate}.xlsx`,
      );
      ...
      <div className="premium-card p-6 mb-8 border-l-4 border-[#003B73] bg-gray-50/50">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="flex gap-2">
            ...
            >
              <Search size={18} />{" "}
              {fetchingReport ? "Loading..." : "View History"}
            </button>
          </div>
          <div>
            <button
              onClick={() =>
                setViewMode(viewMode === "logs" ? "detailed" : "logs")
              }
      ...
      {viewMode === "logs" ? (
          ...
          {attendance.length === 0 ? (
            ...
            <div className="space-y-3">
              {attendance.map((record, idx) => (
                <div
                  key={idx}
                  className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-gray-50 rounded-xl border border-gray-100 gap-4"
                >
        ...
        <div className="animate-fadeIn">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 p-4 bg-[#003B73]/5 rounded-2xl border border-[#003B73]/10">
            <div>
              <h4 className="text-lg font-black text-[#003B73] uppercase tracking-tight">
          ...
          {viewMode === "detailed" &&
            detailedReport &&
            detailedReport.students &&
            detailedReport.students.length > 0 ? (
              ...
              <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {subject?.code} - {subject?.name} | Total Periods Conducted:{" "}
                    <span className="font-bold text-blue-600">
                      {detailedReport.totalPeriodsConducted}
                    </span>
                  </p>
                </div>
              </div>
                    ...
                    {detailedReport.students?.map((student) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-center font-semibold text-purple-500">
                            {student.od}
                          </td>
                          <td
                            className={`px-6 py-4 text-center font-bold ${colorClass}`}
                          >
                            {student.percentage}%
                          </td>
                        </tr>
...
const ClassDetails = () => {
  ...
  useEffect(() => {
    ...
    fetchDetails();
  }, [subjectId, sectionQuery, deptQuery]);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-[#003B73] rounded-2xl p-8 text-white shadow-lg mb-8 animate-fadeIn">
        ...
        <p className="opacity-90 font-mono text-lg mb-6">
          {details.subject.code} • {details.subject.department} -{" "}
          {details.subject.year} Year
        </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {details.totalEstimatedClasses || 45}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="min-h-[400px]">
        ...
        {activeTab === "students" && (
          <StudentsTab subjectId={subjectId} details={details} />
        )}
        {activeTab === "attendance" && (
          <AttendanceTab subjectId={subjectId} details={details} />
        )}
        {activeTab === "marks" && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            ...
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              Enter, update, and manage CIA marks for this class. Marks are
