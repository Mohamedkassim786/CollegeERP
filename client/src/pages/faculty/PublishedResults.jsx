  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setError(
        ...
        "Error fetching results. They may not be published yet.",
      );
      setStudents([]);
    } finally {
      setLoading(false);
          ...
          <div className="flex items-end pb-0.5">
            <div className="w-full md:w-auto">
              <button
                ...
                className="bg-[#003B73] text-white px-10 h-[52px] rounded-[14px] hover:bg-[#002850] transition-all flex items-center justify-center gap-3 font-black shadow-lg shadow-blue-900/10 whitespace-nowrap active:scale-95"
              >
                <Search size={20} strokeWidth={3} /> VIEW PUBLISHED RESULTS
              </button>
            </div>
          </div>
        </div>
              ...
              {students.map((s) => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 font-mono text-sm text-[#003B73] font-bold uppercase">
                      {s.rollNo}
                    </td>
